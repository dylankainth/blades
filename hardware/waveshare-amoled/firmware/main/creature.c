...
#define S(px)           CREATURE_PX(px)    // design pixels (BOX-3) -> this panel
#define CENTER          (CREATURE_SIZE / 2)
#define FRAME_MS        33
#define DOT_COUNT       3
#define RING_COUNT      2

#define BODY            S(84)              // face edge length
#define CORNER_PCT      30                 // corner radius, % of edge (as in the app)
#define EYE_W           S(8)
#define EYE_H           S(19)
#define EYE_DX          S(8)               // eye centre offset from face centre
#define EYE_Y           S(-4)
#define MOUTH_H         S(4)
#define MOUTH_Y         S(13)
#define DOT_D           S(8)
...
#define LOOK_RANGE      S(4)               // px the eyes wander
...
static const int32_t ORBIT_R[DOT_COUNT] = { S(44), S(50), S(56) };   // app: 1.05 / 1.19 / 1.33 x face
static const uint32_t ORBIT_PERIOD_MS[DOT_COUNT] = { 3000, 4000, 5000 };
static const int32_t MOUTH_TALK_W[] = { S(9), S(21), S(13), S(24) }; // app's talk frames
...
static int32_t s_mouth_w16 = S(12) * 16;
...
{
    return (t_ms % BLINK_EVERY_MS) < BLINK_LEN_MS ? S(2) : EYE_H;
}
...
static frame_t frame_chat(uint32_t t)
{
    ...
    for (int i = 0; i < DOT_COUNT; i++) {
        ...
        f.dot_r[i] = ORBIT_R[i] + osc(t + i * 230, 700, S(5));
    }
    return f;
}
...
{
    frame_t f = frame_idle(t);
    const int32_t lift = -LV_ABS(osc(t, 1200, S(12)));
    f.eye_h = S(6);
    f.eye_dx = osc(t, 1600, S(5));
    f.eye_dy = S(1);
    f.mouth_w = S(8);
    f.body_dy = S(2);
    f.dot_deg[0] = 180;
...
static frame_t frame_celebrate(uint32_t t)
{
    ...
    f.body_dy = -LV_ABS(osc(t, 520, S(9)));       // little hops
    for (int i = 0; i < DOT_COUNT; i++) {
        f.dot_deg[i] = spin(t, 800) + i * 120;
        f.dot_r[i] = ORBIT_R[i] + osc(t, 600, S(10));
        f.dot_d[i] = DOT_D + S(2) + osc(t + i * 100, 300, S(3));
    }
    return f;
}
...
lv_obj_t *creature_create(lv_obj_t *parent)
{
    ...
    for (int i = 0; i < RING_COUNT; i++) {
        s_rings[i] = make_shape(s_root, S(2), false);
    }
    for (int i = 0; i < DOT_COUNT; i++) {
        s_dots[i] = make_shape(s_root, 0, true);
    }
    s_chat_arc = make_arc(s_root, S(116), S(5), 0, 80);
    ...
    for (int i = 0; i < 2; i++) {
        ...
        s_happy_eyes[i] = make_arc(s_body, S(16), S(4), 180, 360);   // upward arc: closed, smiling
        lv_obj_align(s_happy_eyes[i], LV_ALIGN_CENTER, side * (EYE_DX + S(3)), EYE_Y + S(5));
        s_blush[i] = make_shape(s_body, 0, true);
        lv_obj_set_size(s_blush[i], S(10), S(10));
        lv_obj_align(s_blush[i], LV_ALIGN_CENTER, side * S(25), MOUTH_Y - S(1));
    ...
    s_smile = make_arc(s_body, S(26), S(4), 25, 155);                // lower arc: a smile
    lv_obj_align(s_smile, LV_ALIGN_CENTER, 0, MOUTH_Y - S(9));
